...
public void update(Observable obj, Object arg)
{
	...
	case TouchEvent.MOVE:
	{
		...
		d = dist(sx1, sy1, x1, y1);
		max_dist = Math.max(max_dist, dist(sx1, sy1, x1, y1));
		if (_mode == MODE_READY)
		{
			if (_idNum == 1)
			{
				...
				if (d > dragThreshold)
				{
					...
					contacts = _numDown;
					IldaEvent.StartMove ie = ildaEvent.new StartMove(sx1, sy1, contacts);
					setChanged(); notifyObservers(ie);
					IldaEvent.Move iem = ildaEvent.new Move(x1, y1, contacts);
					setChanged(); notifyObservers(iem);
				}
			}
			else if (_idNum == 2)
			{
				double sd = dist(sx1, sy1, sx2, sy2);
				double nd = dist(x1, y1, x2, y2);
				double pd = Math.abs(nd-sdelta);
				//System.out.println("Pinch check: " + d + " > " + pinchThreshold);
				if (pd > pinchThreshold && pd >= d/2 && _numDown <= 4)
				{
					_mode = MODE_PINCH;
					// FIXME: compute angle
				...
				else if (d > 2*dragThreshold && sd < 0.03)
				{
					_mode = MODE_MOVE;
					// do drag x1 y1 vs. sx1, sx2
					contacts = _numDown;
		...
		else if (_mode == MODE_MOVE)
		{
			...
			IldaEvent.Move ie = ildaEvent.new Move(x1, y1, contacts);
			setChanged(); notifyObservers(ie);
	...
	case TouchEvent.UP:
	{
		...
		if ((e.id == _firstId) || (_idNum == 2 && e.id == _secondId))
		{
			if (_mode == MODE_MOVE)
			{
				IldaEvent.EndMove ie = ildaEvent.new EndMove(cx1, cy1, contacts);
				setChanged(); notifyObservers(ie);
